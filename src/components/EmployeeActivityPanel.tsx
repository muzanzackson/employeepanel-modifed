...
const EmployeeListPanel: React.FC<EmployeeListPanelProps> = ({ onSelectEmployee }) => {
  ...
  const employees = Object.entries(employeeData).map(([id, data]) => {
    ...
    const latestSentiment = data.Sentiment.length > 0 
      ? [...data.Sentiment].sort((a, b) => 
    ...
    const latestPerformance = data.Performance.length > 0 
    ...
    return {
      id,
      name,
      role: latestPerformance ? latestPerformance.Manager_Feedback : 'New Employee',
      department,
      status,
      ...
      needsAttention: latestSentiment && 
          ...
          >
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                employee.id === 'EMP0125' ? 'bg-pink-600' :
                employee.id === 'EMP0491' ? 'bg-yellow-600' :
                employee.id === 'EMP0310' ? 'bg-green-600' :
                employee.id === 'EMP0172' ? 'bg-blue-600' :
                'bg-purple-600'
              }`}>
                <User size={16} />
              </div>
              <div className="ml-4">
                ...
                <p className="text-sm text-gray-400">{employee.id} • {employee.department}</p>
              </div>
            </div>
            <div className="flex items-center">
              <div className="flex items-center mr-4">
                <div className={`w-2 h-2 rounded-full ${getStatusColor(employee.status)} mr-2`}></div>
                <span className="text-sm text-gray-400 capitalize">{employee.status}</span>
              </div>
              {employee.sentiment !== null && (
                <div className="flex items-center mr-4">
                  <div className={`px-2 py-0.5 rounded-full text-xs ${
                    employee.sentiment >= 4 ? 'bg-green-500/20 text-green-300' :
                    employee.sentiment >= 3 ? 'bg-blue-500/20 text-blue-300' :
